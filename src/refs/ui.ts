...
export function mountRefs(root: HTMLElement, host: RefsHost): RefsPage {
  ...
  function paintStatus() {
    ...
    const shown = S.cells.length - hiddenCells().filter(Boolean).length; // not the ones whose picture won't load
    ...
    else if (x.nearest) st.append(`No close match for this pose · ${shown} nearest from ${srcs} source${srcs > 1 ? 's' : ''} · drag a dot or add words to steer it`);
    else if (!x.exhausted) st.append(`${shown} references from ${srcs} source${srcs > 1 ? 's' : ''} · scroll for more${tuned}${model}`);
    else st.append(`${shown} references from ${srcs} source${srcs > 1 ? 's' : ''}${tuned}`);
  }
  function narrowRow() {
