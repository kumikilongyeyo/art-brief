...
- Entry fields: `id` (kebab-case, unique in the table), `text` (required), and optionally `label`, `weight` (integer 1–10, omit when 5), `tags`, `themes`, `requires`, `excludes`, `places`, `notPlaces`, `surreal`, `tier`, `group`. No other fields (lore tables add their own, see `docs/lore-authoring.md`).
...
- **places / notPlaces** (creature, building, scene only): lines that assume a kind of place. They are checked against the card's **place lines only** — creature `habitat`, building `setting`, scene `location` (+ `time` for later slots) — never against tags from materials or styles, which is why `requires: ["water"]` let "ship lanterns" light a Shadowfell moor (the material was tagged water). `places` = the place needs one of these tags; `notPlaces` = it may have none. Examples: `light` "harsh noon light" → `notPlaces: ["subterranean","cavern"]`; `time` "timeless dark, lit only by fungus glow" → `places: ["subterranean","cavern","drow"]`; `behaviour` "migrates across the sky" → `notPlaces` underground and undersea tags. Tags must exist on that category's place lines; the validator also makes sure every place keeps ≥ 3 lines in the table. It works both ways: a new place line must suit the place rules of lines already on the card (a locked "timeless dark" time keeps Generate underground), and rerolling a place re-picks any unlocked line that no longer fits.
